...
const userLink = args[0]
  ...
  await page.goto("https://www.tiktok.com/@robin.kbr"); //change this to user url page
...
//loop on snaptik for no watermark tiktok videos
    ...
    let current_name = String(urls.length-i).padStart(4, '0')+ "\\" + videoDes[i];
    await page.waitForTimeout(getRandomNumber());
    ...
    const request = https.get(content, function(response) {
        if (response.statusCode === 200) {
            var file = ds.createWriteStream(path+current_name+'.mp4');
            response.pipe(file);
